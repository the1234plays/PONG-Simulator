...
function drawEndScreen() { // Was going to do this properly with a home screen, gameplay, and end screen but I ran out of time
    font('40px Arial')
    fill("black")
    text("Congrats you win", 50, 150, "fill")
    ...
    font('16px Arial');
    fill("black");
    text("Game tester Samuel Oke", 220, 290, "fill");
}
            ...
            if (b.status == 1) { //Had to google status because with out it my code did not work
                if (x > b.x && x < b.x + brickWidth && y > b.y && y < b.y + brickHeight) {
                    ...
                    if (score2 == 24) {
                        ...
                        levels++
                        if (levels == 2) { // // Was going to do this properly with a home screen, gameplay, and end screen but I ran out of time
                          setInterval(drawEndScreen(), 10000)
...
function countTimer() { // Had to google most of this but had to do some problem solivng to make tow okr
    if (pause == false) {
        totalSeconds++;
